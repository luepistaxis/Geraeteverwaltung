...
class Ausgabe(tk.Frame):
    ...
    def ausgabe(self):
        ...
        mask2_title = tk.Label(ausgabe_frame, fg="black", bg='white', text="Mitarbeiterausgabe", font=('Arial', 14))
        mask2_title.place(x=10, y=10)

        mask2_date = tk.Label(ausgabe_frame, fg="black", bg='white', text="Datum:", font=('Arial', 12))
        mask2_date.place(x=40, y=50)
        mask2_dateEntry = DateEntry(ausgabe_frame, date_pattern='dd.mm.yyyy', borderwidth=1, relief='solid', locale='de_DE')
        mask2_dateEntry.place(x=100, y=50)

        mask2_mitarbeiterRadioBtn = tk.Radiobutton(ausgabe_frame, text="Mitarbeiter", font=('Arial', 12), bg="white", variable=mask2_auswahl_var, value="Mitarbeiter")
        mask2_mitarbeiterRadioBtn.place(x=40, y=80)

        mask2_arbeitsplatzRadioBtn = tk.Radiobutton(ausgabe_frame, text="Arbeitsplatz", font=('Arial', 12), bg="white", variable=mask2_auswahl_var, value="Arbeitsplatz")
        mask2_arbeitsplatzRadioBtn.place(x=140, y=80)

        
        mask2_auswahl = ttk.Combobox(ausgabe_frame, textvariable=mask2_auswahl_var)
        ...
        mask2_device = tk.Label(ausgabe_frame, text="Geräte:", fg="black", bg='white', font=('Arial', 12))
        mask2_device.place(x=40, y=170)

        mask2_speichern = tk.Button(ausgabe_frame, text="speichern", font=("Arial", 16), command=speichern)
        mask2_speichern.place(x=700, y=500)

        mask2_verwerfen = tk.Button(ausgabe_frame, text="verwerfen", font=("Arial", 16), command=verwerfen)
        mask2_verwerfen.place(x=700, y=10)

        #mask2_hinzufügen = tk.Button(ausgabe_frame, text="+", font=('Arial', 16), command=open_new_frame)
        #mask2_hinzufügen.place(x=500, y=170, height=30, width=30)

        #Frame zur Wahl von Geräten
        mask2_deviceFrame = tk.Frame(ausgabe_frame, borderwidth=1, relief="solid")
